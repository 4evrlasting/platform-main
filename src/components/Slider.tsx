...
export function SlideSection(props: React.PropsWithChildren) {
  ...
  const _scroll = () => {
    ...
    leftArrow.current.style.display = target.scrollLeft > 6 ? 'block' : 'none'
    rightArrow.current.style.display = target.scrollLeft + target.clientWidth < target.scrollWidth - 6 ? 'block' : 'none'
  }
